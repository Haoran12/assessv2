...
func NewWithDatabases(cfg config.Config, businessDB *gorm.DB, accountDB *gorm.DB) *gin.Engine {
	...
	api := r.Group("/api")
	{
		...
		system.GET("/settings", middleware.RequirePermission("setting:view"), settingsHandler.List)
		system.PUT("/settings", middleware.RequirePermission("setting:update"), settingsHandler.Update)
